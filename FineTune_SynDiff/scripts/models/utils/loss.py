...
def convert_to_one_channel(output_test, batch_size, hori_translation, verti_translation, image_size):
    hori_translation  = hori_translation.repeat(batch_size,1,1,1).cuda()
    verti_translation = verti_translation.repeat(batch_size,1,1,1).cuda()    
    output_test = F.sigmoid(output_test)
    class_pred = output_test.view([batch_size,-1,8,image_size,image_size]) #(B, C, 8, H, W)
