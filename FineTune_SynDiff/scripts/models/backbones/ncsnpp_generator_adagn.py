...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config, seg = False, is_three_inputs = False):
    super().__init__()
    self.config = config
    if seg == False:
        self.not_use_tanh = False
    if seg == True:
        self.not_use_tanh = True #True
    ...
    if is_three_inputs == True:
        channels = config.num_channels  + 1
    if seg == True and is_three_inputs == False:
    ...
    if progressive != 'output_skip':
      modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 32),
                                  num_channels=in_ch, eps=1e-6))
      if is_three_inputs == True:
          modules.append(conv3x3(in_ch, channels - 1, init_scale=init_scale))
      if seg == True and is_three_inputs == False:
          modules.append(conv3x3(in_ch, channels + 6, init_scale=init_scale))          
  ...
  def forward(self, x, time_cond, z):
    ...
    aux = self.channel_mapping(h)
    aux = F.interpolate(aux,scale_factor=32,mode='bilinear',align_corners=True)
    aux = self.direc_reencode(aux)
    pyramid = None
    ...
    if not self.not_use_tanh:
        return torch.cat((torch.tanh(h[:, 0, :]).unsqueeze(1), h[:, 1, :].unsqueeze(1)), axis = 1)
    else:
        return h, aux
