...
def Bilateral_voting(c_map, hori_translation, verti_translation):
    ...
    vote_out = torch.zeros([batch,class_num,channel, row, column]).to(str(c_map.device))
    # print(vote_out.shape,c_map.shape,hori_translation.shape)
    right = (torch.bmm(c_map[:,:,4].contiguous().view(-1,row,column),hori_translation.view(-1,column,column))).view(batch,class_num,row,column).to(str(c_map.device))

    left = (torch.bmm(c_map[:,:,3].contiguous().view(-1,row,column),hori_translation.transpose(3,2).view(-1,column,column))).view(batch,class_num,row,column).to(str(c_map.device))

    left_bottom = (torch.bmm(verti_translation.transpose(3,2).view(-1,row,row), c_map[:,:,5].contiguous().view(-1,row,column))).view(batch,class_num,row,column).to(str(c_map.device))
    left_bottom = (torch.bmm(left_bottom.view(-1,row,column),hori_translation.transpose(3,2).view(-1,column,column))).view(batch,class_num,row,column).to(str(c_map.device))
    right_above = (torch.bmm(verti_translation.view(-1,row,row), c_map[:,:,2].contiguous().view(-1,row,column))).view(batch,class_num,row,column).to(str(c_map.device))
    right_above= (torch.bmm(right_above.view(-1,row,column),hori_translation.view(-1,column,column))).view(batch,class_num,row,column).to(str(c_map.device))
    left_above = (torch.bmm(verti_translation.view(-1,row,row), c_map[:,:,0].contiguous().view(-1,row,column))).view(batch,class_num,row,column).to(str(c_map.device))
    left_above = (torch.bmm(left_above.view(-1,row,column),hori_translation.transpose(3,2).view(-1,column,column))).view(batch,class_num,row,column).to(str(c_map.device))
    bottom = (torch.bmm(verti_translation.transpose(3,2).view(-1,row,row), c_map[:,:,6].contiguous().view(-1,row,column))).view(batch,class_num,row,column).to(str(c_map.device))
    up = (torch.bmm(verti_translation.view(-1,row,row), c_map[:,:,1].contiguous().view(-1,row,column))).view(batch,class_num,row,column).to(str(c_map.device))
    right_bottom = (torch.bmm(verti_translation.transpose(3,2).view(-1,row,row), c_map[:,:,7].contiguous().view(-1,row,column))).view(batch,class_num,row,column).to(str(c_map.device))
    right_bottom = (torch.bmm(right_bottom.view(-1,row,column),hori_translation.view(-1,column,column))).view(batch,class_num,row,column).to(str(c_map.device))
    ...
    # print("right", right.unique())
    # print("left", left.unique())
    # print("vote_out", vote_out.unique())
    # print("right_bottom", right_bottom.unique())
    # print("up", up.unique())
    # print("left_above", left_above.unique())
    # print("right_above", right_above.unique())
    # print("left_bottom", left_bottom.unique())
    # print("pred_mask", pred_mask.unique())
...
def convert_to_one_channel(output_test, batch_size, hori_translation, verti_translation, image_size):
    output_test = F.sigmoid(output_test).to(str(output_test.device))
    hori_translation  = hori_translation.repeat(batch_size,1,1,1).to(str(output_test.device))
    verti_translation = verti_translation.repeat(batch_size,1,1,1).to(str(output_test.device))
    class_pred = output_test.view([batch_size,-1,8,image_size,image_size]).to(str(output_test.device)) #(B, C, 8, H, W)
    pred = torch.where(class_pred>0.5,1,0).to(device=str(output_test.device), dtype=torch.float32)
    # print("pred1", pred.unique())
    pred,_ = Bilateral_voting(pred, hori_translation, verti_translation)    
    # print("output_test", output_test.unique())
    # print("hori_translation", hori_translation.unique())
    # print("verti_translation", verti_translation.unique())
    # print("class_pred", class_pred.unique())
    # print("pred2", pred.unique())
    return pred

class connect_loss(nn.Module):
    def __init__(self,hori_translation,verti_translation,density=None,bin_wide=None):
